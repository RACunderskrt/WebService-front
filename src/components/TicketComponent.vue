    ...
    <!-- Route -->
		<div class="card-route">
			<div class="route-point">
				<span class="route-code">{{ flight.departureAirport }}</span>
			...
			<div class="route-point route-point--right">
				<span class="route-code">{{ flight.arrivalAirport }}</span>
    ...
    <!-- Dates -->
		<div class="card-dates">
			<div class="date-block">
				<span class="date-value">{{ formatDate(flight.departureDate) }}</span>
				<span class="date-time">{{ formatTime(flight.departureDate) }}</span>
			</div>
			<div class="duration-block">
				<span class="duration">{{ duration }}</span>
			</div>
			<div class="date-block date-block--right">
				<span class="date-value">{{ formatDate(flight.arrivalDate) }}</span>
				<span class="date-time">{{ formatTime(flight.arrivalDate) }}</span>
			</div>
		</div>
...
const duration = computed(() => {
	const diff = (new Date(props.flight.arrivalDate) - new Date(props.flight.departureDate)) / 60000
